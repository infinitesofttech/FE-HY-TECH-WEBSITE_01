      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pt-4">
          {categories.map((cat, i) => {
            ...
            const categoryUrl = `/services?category=${cat.slug || cat.id}`;
            ...
            return (
              ...
              >
                <Link
                  to={categoryUrl}
                  className={`w-full h-[280px] sm:h-[300px] rounded-[24px] ${palette.bg} ${palette.border} border p-6 flex flex-col justify-between hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 group block cursor-pointer select-none`}
                >
                  {/* Top section: Icon */}
                  <div>
                    <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm mb-6 group-hover:scale-110 group-hover:shadow-md transition-all duration-300">
                      <Icon size={26} className={palette.text} />
                    </div>
                    
                    {/* Title and Count */}
                    <h3 className="font-bold text-[#171717] text-xl md:text-2xl mb-1 group-hover:text-black transition-colors">
                      {t(cat.title)}
                    </h3>
                  ...
                  <div className="flex items-center justify-between w-full mt-auto pt-4">
                    <span className="font-bold text-[13px] text-[#171717] group-hover:text-black transition-colors">
                      Explore Services
                    </span>
                    <div 
                      className="w-9 h-9 rounded-full bg-white shadow-sm flex items-center justify-center btn-3d-circle border border-gray-100 group-hover:scale-115 group-hover:rotate-6 transition-all duration-300"
                    >
