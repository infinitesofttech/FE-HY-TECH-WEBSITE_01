/**
 * HY-TECH Computer Education & Online Hub
 * Complete 3D Service Visuals Mapping & Resilient Fallback Utility
 */

export const categoryVisuals = {
  'online-services': '/images/categories/online_services_antigravity.jpg',
  'education-services': '/images/categories/education_services_antigravity.jpg',
  'job-services': '/images/categories/job_services_antigravity.jpg',
  'printing-services': '/images/categories/printing_services_antigravity.jpg',
  'computer-courses': '/images/categories/computer_courses_antigravity.jpg',
  'other-services': '/images/categories/other_services_antigravity.jpg',
  default: '/images/categories/online_services_antigravity.jpg',
  'election-card': '/images/services/election-card.webp',
  'abha-card': '/images/services/abha-card.webp',
  'ration-card': '/images/services/ration-card.webp',
  'ration-card-kyc': '/images/services/ration-card.webp',
  'awas-yojana': '/images/services/awas-yojana.webp',
  'pmay-awas-yojana': '/images/services/awas-yojana.webp',
  'udyam-registration': '/images/services/udyam-registration.webp',
  'citizen-new-registration': '/images/services/citizen-portal-registration.webp',
  'citizen-portal-registration': '/images/services/citizen-portal-registration.webp',
  'navodaya-yojana': '/images/services/navodaya-admission.webp',
  'sbi-scholarship': '/images/services/sbi-scholarship.webp',
  'education-forms': '/images/services/education-exam-services.webp',
  'bus-pass-form': '/images/services/bus-pass-new.webp',
  'bus-pass': '/images/services/bus-pass-new.webp',
  'bus-pass-new': '/images/services/bus-pass-new.webp',
  'pf-kyc': '/images/services/pf-kyc.webp',
  'pf-transfer': '/images/services/pf-transfer.webp',
  'pf-withdrawal': '/images/services/pf-withdrawal.webp',
  'pf-advance': '/images/services/pf-advance.webp',
  'pension-claim': '/images/services/pf-pension-claim.webp',
  'pf-pension-claim': '/images/services/pf-pension-claim.webp',
  'pf-status': '/images/services/pf-claim-status.webp',
  'pf-claim-status': '/images/services/pf-claim-status.webp',
  'pf-name-correction': '/images/services/pf-name-correction.webp',
  'pf-dob-correction': '/images/services/pf-dob-correction.webp',
  'pf-profile-update': '/images/services/pf-profile-update.webp',
  'pf-e-nomination': '/images/services/pf-e-nomination.webp',
  'passport-new': '/images/services/passport.webp',
  'passport-renewal': '/images/services/passport-renewal.webp',
  'kunvarbai-mameru': '/images/services/kunvarbai-mameru.webp',
  'driving-licence-renewal': '/images/services/driving-license-renewal.webp',
  'driving-license-renewal': '/images/services/driving-license-renewal.webp',
  'food-license': '/images/services/food-license-new.webp',
};

// All 60 service visuals with 100% unique dedicated photography
export const serviceVisuals = {
  // 1. ONLINE SERVICES
  'pan-card': '/images/services/pan-card.webp',
  'aadhaar': '/images/services/aadhaar-card.webp',
  'aadhaar-card': '/images/services/aadhaar-card.webp',
  'voter-id': '/images/services/election-card.webp',
  'passport': '/images/services/passport.webp',
  'driving-licence': '/images/services/driving-licence.webp',
  'ayushman-card': '/images/services/ayushman-card.webp',
  'e-shram': '/images/services/e-shram.webp',
  'e-shram-card': '/images/services/e-shram.webp',
  'pm-kisan': '/images/services/pm-kisan.webp',
  'income-certificate': '/images/services/income-certificate.webp',
  'caste-certificate': '/images/services/caste-certificate.webp',
  'ncl-certificate': '/images/services/ncl-certificate.webp',
  'non-creamy-layer': '/images/services/ncl-certificate.webp',
  'ews-certificate': '/images/services/ews-certificate.webp',
  'domicile-certificate': '/images/services/domicile-certificate.webp',
  'birth-certificate': '/images/services/birth-certificate.webp',
  'death-certificate': '/images/services/death-certificate.webp',
  'certificate': '/images/services/income-certificate.webp',

  // 2. EDUCATION SERVICES
  'college-admission': '/images/services/college-admission.webp',
  'vanraj-college-admission': '/images/services/vanraj-college-admission.webp',
  'cgas-registration': '/images/services/cgas-registration.webp',
  'university-exam': '/images/services/university-exam.webp',
  'university-exam-form': '/images/services/university-exam.webp',
  'university-registration': '/images/services/university-registration.webp',
  'hostel-admission': '/images/services/hostel-admission.webp',
  'scholarship': '/images/services/scholarship.webp',
  'scholarship-form': '/images/services/scholarship.webp',
  'college-exam-renewal': '/images/services/college-exam-renewal.webp',
  'college-renewal': '/images/services/college-exam-renewal.webp',
  'resume-cv': '/images/services/resume-cv.webp',
  'resume-cv-making': '/images/services/resume-cv.webp',

  // 3. JOB SERVICES
  'government-job': '/images/services/government-job.webp',
  'gov-job': '/images/services/government-job.webp',
  'railway-recruitment': '/images/services/railway-recruitment.webp',
  'railway': '/images/services/railway-recruitment.webp',
  'railway-job': '/images/services/railway-recruitment.webp',
  'ssc-recruitment': '/images/services/ssc-recruitment.webp',
  'ssc-job': '/images/services/ssc-recruitment.webp',
  'upsc-recruitment': '/images/services/upsc-recruitment.webp',
  'upsc-job': '/images/services/upsc-recruitment.webp',
  'gpsc-recruitment': '/images/services/gpsc-recruitment.webp',
  'gpsc-job': '/images/services/gpsc-recruitment.webp',
  'ssc-upsc-gpsc': '/images/services/gpsc-recruitment.webp',
  'police-recruitment': '/images/services/police-recruitment.webp',
  'army-recruitment': '/images/services/army-recruitment.webp',
  'defense-clerk': '/images/services/police-recruitment.webp',
  'talati-recruitment': '/images/services/talati-recruitment.webp',
  'clerk-recruitment': '/images/services/clerk-recruitment.webp',
  'bank-recruitment': '/images/services/bank-recruitment.webp',
  'apprentice-recruitment': '/images/services/apprentice-recruitment.webp',

  // 4. PRINTING SERVICES
  'xerox': '/images/services/xerox.webp',
  'xerox-photocopy': '/images/services/xerox.webp',
  'printout': '/images/services/printout.webp',
  'colour-printing': '/images/services/colour-printing.webp',
  'bw-printing': '/images/services/printout.webp',
  'xerox-print': '/images/services/xerox.webp',
  'lamination': '/images/services/lamination.webp',
  'scanning': '/images/services/scanning.webp',
  'document-scanning': '/images/services/scanning.webp',
  'spiral-binding': '/images/services/spiral-binding.webp',
  'pvc-card': '/images/services/pvc-card.webp',
  'pvc-card-printing': '/images/services/pvc-card.webp',
  'passport-photo': '/images/services/passport-photo.webp',
  'photo-printing': '/images/services/photo-printing.webp',
  'document-printing': '/images/services/document-printing.webp',

  // 5. COMPUTER COURSES
  'ccc-course': '/images/services/ccc-course.webp',
  'basic-computer': '/images/services/basic-computer.webp',
  'ms-office': '/images/services/ms-office.webp',
  'advanced-excel': '/images/services/advanced-excel.webp',
  'tally-gst': '/images/services/tally-gst.webp',
  'tally-prime-gst': '/images/services/tally-gst.webp',
  'canva-design': '/images/services/canva-design.webp',
  'advanced-courses': '/images/services/tally-gst.webp',
  'typing-course': '/images/services/typing-course.webp',
  'ai-tools-course': '/images/services/ai-tools-course.webp',
  'python-programming': '/images/services/python-programming.webp',
  'web-development': '/images/services/web-development.webp',
  'internet-training': '/images/services/internet-training.webp',

  // 6. OTHER SERVICES
  'money-transfer': '/images/services/money-transfer.webp',
  'mobile-recharge': '/images/services/mobile-recharge.webp',
  'bill-payment': '/images/services/bill-payment.webp',
  'electricity-bill-payment': '/images/services/electricity-bill-payment.webp',
  'water-bill-payment': '/images/services/water-bill-payment.webp',
  'train-ticket': '/images/services/train-ticket.webp',
  'train-ticket-booking': '/images/services/train-ticket.webp',
  'bus-ticket': '/images/services/bus-ticket.webp',
  'bus-ticket-booking': '/images/services/bus-ticket.webp',
  'flight-ticket': '/images/services/flight-ticket.webp',
  'flight-ticket-booking': '/images/services/flight-ticket.webp',
  'international-flight': '/images/services/international-flight.webp',
  'ticketing': '/images/services/train-ticket.webp',
  'banking-bill': '/images/services/money-transfer.webp',
  'online-form-assistance': '/images/services/online-form-assistance.webp',
};

// All 45 services now have individual dedicated 3D visuals
export const availableCustomVisuals = new Set(Object.keys(serviceVisuals));

/**
 * Returns the best visual URL for a service with category fallback.
 */
export function getServiceVisual(slug, categoryId) {
  if (slug && serviceVisuals[slug]) {
    return serviceVisuals[slug];
  }
  return categoryVisuals[categoryId] || categoryVisuals.default;
}

/**
 * Handles image loading error and sets fallback
 */
export function handleImageFallback(e, categoryId) {
  e.target.onerror = null;
  e.target.src = categoryVisuals[categoryId] || categoryVisuals.default;
}
